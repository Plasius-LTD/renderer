...
function validatePackageDirectory(root, packageDirectory) {
  ...
  const absolute = path.resolve(absoluteRoot, packageDirectory);
  ...
  if (directoryStats.isSymbolicLink() || !directoryStats.isDirectory()) {
    throw new Error(`Package directory is not a regular directory: ${packageDirectory}`);
  }
...
function validatePackage(root, packageDirectory, execute = run) {
...
function main(
  ...
  if (!options.sourceOnly) {
    const validate = dependencies.validatePackage ?? validatePackage;
    for (const packageDirectory of options.packageDirectories) {
      validate(root, packageDirectory);
    }
  }
  ...
  return `Public artifact integrity passed (${suffix}; contents of the prohibited path were not read).`;
}
